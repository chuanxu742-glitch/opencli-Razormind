...
async function mockKnowledge(page) {
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    if (path.endsWith('/knowledge-libraries')) return route.fulfill(envelope([{ id: 'lib', name: '高吉星', workspace_id: 'w', legacy_brand_id: 'g' }, { id: 'other', name: '其他资料', workspace_id: 'w', legacy_brand_id: null }]))
...
test('知识资料上传、检索与问答携带产品范围，切换产品清除旧回答', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: '资料库', exact: true }).selectOption('other')
...
test('项目按产品归类，记录筛选时禁用全站清空', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '清空全部', exact: true })).toHaveCount(0)
  await expect.poll(() => requests.some(item => item.path.endsWith('/records') && item.search.includes('workspace_id=w'))).toBe(true)
})
...
test('未保存正文切换产品时提醒，取消后保留输入', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: '资料库', exact: true })).toHaveValue('lib')
  await page.getByRole('button', { name: '保存草稿', exact: true }).click()
