...
@pytest.mark.asyncio
async def test_pipeline_account_resolution_commits_fence_for_next_session(db_engine, db_session):
    """A real resolver mutation survives into a separately opened DB session."""
    ...
    with patch("backend.database.AsyncSessionLocal", _sessionmaker(db_engine)):
        ref, envelope = await _resolve_account_execution(
            source,
            {"execution_id": "execution-1", "caller_id": "caller-1"},
        )
    ...
    assert persisted.node_boot_id == node.boot_id
