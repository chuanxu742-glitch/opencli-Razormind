...
def test_capacity_experiment_rejects_unbounded_query_parameters():
    with pytest.raises(CapacityConfigurationError, match="accounts"):
        build_config("postgresql://user:pass@localhost/db", accounts=10_000_001, page_size=10, claim_size=10, samples=1)
    with pytest.raises(CapacityConfigurationError, match="page_size"):
        build_config("postgresql://user:pass@localhost/db", accounts=10, page_size=101, claim_size=10, samples=1)
    with pytest.raises(CapacityConfigurationError, match="claim_size"):
        build_config("postgresql://user:pass@localhost/db", accounts=10, page_size=10, claim_size=101, samples=1)
