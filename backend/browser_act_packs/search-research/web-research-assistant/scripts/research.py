...
def main():
    ...
    parser.add_argument(
        "--max-results", "-m", type=int, default=10, help="Maximum number of results (1-50)"
    )
    parser.add_argument(
        "--content-type",
        "-c",
    ...
    parser.add_argument(
        "--format", "-f", default="markdown", choices=["markdown", "json"], help="Output format"
    )
    ...
    try:
        ...
        results = execute_web_search(
            args.query, args.engine, args.max_results, args.content_type, args.time_range
        )
        ...
        if args.format == "markdown":
            ...
            report = json.dumps(
                {"query": args.query, "results": results, "extracted": extracted},
                indent=2,
                ensure_ascii=False,
            )
