...
def main():
    ...
    (function() {
      try {
        ...
        // Description is the title on current Goofish detail pages.
        var descEl = document.querySelector('[class*="main--"][class*="open"], span[class*="desc--"]');
        var description = descEl ? clean(descEl.textContent) : null;
        ...
        if (!priceEl) {
          priceEl = document.querySelector('[class*="item-main-info"] [class*="price"]');
        }
        var price = priceEl ? clean(priceEl.textContent) : null;

        // Original price from tips area.
        var tipsEl = document.querySelector('[class*="item-main-info"] [class*="tips--"]');
        var originalPrice = null;
        if (tipsEl) {
          var opMatch = (tipsEl.textContent || '').match(/直接买\s*[￥¥]\s*(\d+(?:\.\d{1,2})?)/);
          if (opMatch) originalPrice = opMatch[1];
        }

        // Seller info.
        var nickEl = document.querySelector('[class*="item-user-info-nick"]');
        var sellerName = nickEl ? clean(nickEl.textContent) : null;
        var avatarEl = document.querySelector('[class*="item-user-info-avatar"] img');
        var sellerAvatar = avatarEl ? (avatarEl.currentSrc || avatarEl.src || avatarEl.getAttribute('data-src') || null) : null;
        var labelEls = document.querySelectorAll('[class*="item-user-info-label"]');
        var sellerLabels = Array.from(labelEls).map(function(el) { return clean(el.textContent); }).filter(Boolean);

        // Images from gallery (deduplicate by base filename).
        var imgEls = document.querySelectorAll('[class*="item-main-window"] img');
        if (imgEls.length === 0) {
          imgEls = document.querySelectorAll('[class*="item-main"] img');
        }
        var seen = {};
        var images = [];
        ...
        var tags = Array.from(tagEls).map(function(el) { return clean(el.textContent); }).filter(Boolean);
        ...
        if (wantEl) {
          ...
          wantCount = wantMatch ? wantMatch[1] : clean(wantText);
        }

        var itemUrl = window.location.href;
        var idMatch = itemUrl.match(/[?&]id=([^&#]+)/);
        var categoryMatch = itemUrl.match(/[?&]categoryId=([^&#]+)/);
        if (!description && !price) {
          return JSON.stringify({ error: true, message: 'Item content not loaded — page may still be loading' });
        }

        return JSON.stringify({
          item_id: idMatch ? decodeURIComponent(idMatch[1]) : null,
          category_id: categoryMatch ? decodeURIComponent(categoryMatch[1]) : null,
          item_url: itemUrl,
          title: title,
          price: price,
